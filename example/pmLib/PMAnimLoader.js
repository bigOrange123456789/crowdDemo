...
function PMAnimLoader(manager) {
		...
		for ( var lightId in lights ) {
			...
			if ( lightNode ) {
				...
				if ( light.fallOffExponent !== undefined ) {
					...
					console.warn( 'THREE.PMAnimLoader:: light.fallOffExponent not currently supported.' );
	...
	function GLTFBinaryExtension( data ) {
		...
		if ( this.header.magic !== BINARY_EXTENSION_HEADER_MAGIC ) {
			...
			throw new Error( 'THREE.PMAnimLoader: Unsupported glTF-Binary header.' );

		} else if ( this.header.version < 2.0 ) {

			throw new Error( 'THREE.PMAnimLoader: Legacy binary file detected. Use LegacyPMAnimLoader instead.' );
		...
		if ( this.content === null ) {
			...
			throw new Error( 'THREE.PMAnimLoader: JSON content not found.' );
	...
	function GLTFDracoMeshCompressionExtension ( dracoLoader ) {
		...
		if ( ! dracoLoader ) {
			...
			throw new Error( 'THREE.PMAnimLoader: No DRACOLoader instance provided.' );
	...
	GLTFParser.prototype.loadBuffer = function ( bufferIndex ) {
		...
		if ( bufferDef.type && bufferDef.type !== 'arraybuffer' ) {
			...
			throw new Error( 'THREE.PMAnimLoader: ' + bufferDef.type + ' buffer type is not supported.' );
		...
		return new Promise( function ( resolve, reject ) {
			...
			loader.load( resolveURL( bufferDef.uri, options.path ), resolve, undefined, function () {
				...
				reject( new Error( 'THREE.PMAnimLoader: Failed to load buffer "' + bufferDef.uri + '".' ) );
		...
		return Promise.all( pendingBufferViews ).then( function ( bufferViews ) {
			...
			if ( accessorDef.sparse !== undefined ) {
				...
				for ( var i = 0, il = sparseIndices.length; i < il; i ++ ) {
					...
					if ( itemSize >= 5 ) throw new Error( 'THREE.PMAnimLoader: Unsupported itemSize in sparse BufferAttribute.' );
	...
	GLTFParser.prototype.loadTexture = function ( textureIndex ) {
        ...
        return Promise.resolve( sourceURI ).then( function ( sourceURI ) {} ).then( function ( texture ) {});
		...
		return Promise.resolve( sourceURI ).then( function ( sourceURI ) {
			...
			if ( textureDef.internalFormat !== undefined && texture.format !== WEBGL_TEXTURE_FORMATS[ textureDef.internalFormat ] ) {
				...
				console.warn( 'THREE.PMAnimLoader: Three.js does not support texture internalFormat which is different from texture format. ' +
			...
			return scope.loadGeometries( primitives ).then( function ( geometries ) {
				...
				for ( var i = 0, il = primitives.length; i < il; i ++ ) {
					...
					if ( material.aoMap
						...
						console.log( 'THREE.PMAnimLoader: Duplicating UVs to support aoMap.' );
					...
					if ( primitive.mode === WEBGL_CONSTANTS.TRIANGLES ||
						...
						throw new Error( 'THREE.PMAnimLoader: Primitive mode unsupported: ' + primitive.mode );
	...
	GLTFParser.prototype.loadCamera = function ( cameraIndex ) {
		...
		if ( ! params ) {
			...
			console.warn( 'THREE.PMAnimLoader: Missing camera parameters.' );
			return;
				...
				for ( var i = 0, il = meshes.length; i < il; i ++ ) {
					...
					for ( var j = 0, jl = skinEntry.joints.length; j < jl; j ++ ) {
						...
						if ( jointNode ) {
							...
							console.warn( 'THREE.PMAnimLoader: Joint "%s" could not be found.', jointId );
...
PMAnimLoader.prototype = {

	constructor: PMAnimLoader,
	...
	parse: function ( data, path, onLoad, onError ) {
		...
		if ( json.asset === undefined || json.asset.version[ 0 ] < 2 ) {
			...
			if ( onError ) onError( new Error( 'THREE.PMAnimLoader: Unsupported asset. glTF versions >=2.0 are supported. Use LegacyPMAnimLoader instead.' ) );
			return;
		...
		console.time( 'PMAnimLoader' );

		var GLTFParser=this.GLTFParser;
